...
# ✅ FIXED IMPORTS (Render compatible)
...
# ✅ Frontend directory (outside backend)
BASE_DIR = Path(__file__).resolve().parent.parent
FRONTEND_DIR = BASE_DIR / "frontend"
...
@app.get("/")
async def root():
    """Serve frontend login page or index"""
    login_file = FRONTEND_DIR / "login.html"
    if login_file.exists():
        return FileResponse(login_file)
...
async def save_weather(
    ...
    try:
        ...
        return {"success": True, "message": "Weather data saved successfully"}
...
async def get_hourly_forecast(city: str):
    try:
        data = weather_service.get_hourly_forecast(city)
...
async def get_daily_forecast(city: str):
    try:
        data = weather_service.get_daily_forecast(city)
        return {"success": True, "city": city, "data": data}
...
async def geocode_location(query: str, limit: int = 5):
    try:
        data = weather_service.geocode_location(query, limit)
